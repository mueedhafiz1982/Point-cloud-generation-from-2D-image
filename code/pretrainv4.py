...
util.mkdir("/content/drive/MyDrive/attprj1/3D-point-cloud-generation/models_{0}".format(opt.group))
...
with tf.device("/gpu:0"):
	...
	encoder = graphv4.encoder if opt.arch=="original" else \
			  graphv4.encoder_resnet if opt.arch=="resnet" else None
	decoder = graphv4.decoder if opt.arch=="original" else \
			  graphv4.decoder_resnet if opt.arch=="resnet" else None
	latent = encoder(opt,inputImage)
	XYZ, maskLogit,d1 = decoder(opt,latent) # [B,H,W,3V],[B,H,W,V]
	depth = XYZ[:,:,:,8*2:8*3]
	mask=tf.to_float(maskLogit>1)
	# ------ define loss ------
	...
	#XY2 = XYZ2[:,:,:,:4*2]
	XY=XYZ[:,:,:,:8*2]
	#depth=tf.concat([depth1, depth2], axis=-1)
	#mask=tf.concat([mask1, mask2], axis=-1)
	#maskLogit=tf.concat([maskLogit1, maskLogit2], axis=-1)	
	loss_XYZ = graphv4.l1_loss(XY-XYGT)/opt.batchSize
	loss_XYZ += graphv4.masked_l1_loss(depth-depthGT,maskLogit>0)/opt.batchSize
	loss_mask = graphv4.cross_entropy_loss(maskLogit,maskGT)/opt.batchSize
...
summaryWriter = tf.summary.FileWriter("/content/drive/MyDrive/attprj1/3D-point-cloud-generation/summary_{0}/{1}".format(opt.group,opt.model))
...
with tf.Session(config=tfConfig) as sess:
	...
	for c in range(chunkResumeN,chunkMaxN):
		...
		for i in range(c*opt.itPerChunk,(c+1)*opt.itPerChunk):
			...
			batch[lr_PH] = lr
			# run one step
			graphv4.model_summary()
			#print(depth.shape,depthGT.shape,maskLogit.shape,maskGT.shape,XY.shape,XYGT.shape)
