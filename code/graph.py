...
def decoder(opt,latent):
	...
	def pixelconv2Layer(opt,feat,outDim):
		weight1,bias1 = createVariable1(opt,[1,1,int(feat.shape[-1]),outDim],gridInit=True)
		conv = tf.nn.conv2d(feat,weight1,strides=[1,1,1,1],padding="SAME")+bias1
		return conv    
	with tf.variable_scope("decoder"):
		...
		with tf.variable_scope("pixelconv1"): feat_a = pixelconv2Layer(opt,feat,8*4) # 128x128
		...
		with tf.variable_scope("pixelconv2"): feat_b = pixelconv2Layer(opt,feat,8*4) # 128x128
		feat=tf.add(feat_a, feat_b)
		feat = feat/2
...
def createVariable1(opt,weightShape,biasShape=None,stddev=None,gridInit=False):
	...
	if gridInit:
		...
		initTile = np.concatenate([np.tile(X,[8,1,1]),
								   np.tile(Y,[8,1,1]),
								   np.ones([8,128,128],dtype=np.float32)*opt.renderDepth,
								   np.zeros([8,128,128],dtype=np.float32)],axis=0) # [4V,H,W]
