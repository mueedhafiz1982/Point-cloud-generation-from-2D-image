...
def decoder(opt,latent):
	...
	def pixelconv2Layer1(opt,feat,outDim):
		...
		conv1 = tf.nn.conv2d(feat,weight[:,:,:,0:8],strides=[1,1,1,1],padding="SAME")+bias[:,:,:,0:8]
		conv2 = tf.nn.conv2d(feat,weight[:,:,:,8:16],strides=[1,1,1,1],padding="SAME")+bias[:,:,:,8:16]
		conv3 = tf.nn.conv2d(feat,weight[:,:,:,16:24],strides=[1,1,1,1],padding="SAME")+bias[:,:,:,16:24]
		conv4 = tf.nn.conv2d(feat,weight[:,:,:,24:32],strides=[1,1,1,1],padding="SAME")+bias[:,:,:,24:32]
	...
	with tf.variable_scope("decoder"):
		...
		with tf.variable_scope("pixelconv1"): feat_a,tmp,tmp,tmp = pixelconv2Layer1(opt,feat,8*4) # 128x128
		...
		with tf.variable_scope("pixelconv2"): tmp,feat_b,tmp,tmp = pixelconv2Layer1(opt,feat,8*4) # 128x128
		...
		with tf.variable_scope("pixelconv3"): tmp,tmp,feat_c,tmp = pixelconv2Layer1(opt,feat,8*4) # 128x128
		...
		with tf.variable_scope("pixelconv4"): tmp,tmp,tmp,feat_d = pixelconv2Layer1(opt,feat,8*4) # 128x128

		feat1=tf.concat([feat_a,feat_b], axis=-1)
		feat2=tf.concat([feat_c,feat_d], axis=-1)
