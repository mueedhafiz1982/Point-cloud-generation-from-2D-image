...
util.mkdir("/content/drive/MyDrive/attprj1/3D-point-cloud-generation/results_{0}/{1}".format(opt.group,opt.load))
...
with tf.device("/gpu:0"):
	...
	XYZ,maskLogit,dt = decoder(opt,latent) # [B,H,W,3V],[B,H,W,V]
...
with tf.Session(config=tfConfig) as sess:
	...
	for c in range(chunkN):
		...
		for i in range(testN):
			...
			# output results
			scipy.io.savemat("/content/drive/MyDrive/attprj1/3D-point-cloud-generation/results_{0}/{1}/{2}.mat".format(opt.group,opt.load,CAD),{
