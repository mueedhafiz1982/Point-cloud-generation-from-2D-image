...
with tf.device("/gpu:0"):
	...
	encoder = graphv8.encoder if opt.arch=="original" else \
			  graphv8.encoder_resnet if opt.arch=="resnet" else None
	decoder = graphv8.decoder if opt.arch=="original" else \
			  graphv8.decoder_resnet if opt.arch=="resnet" else None
	...
	loss_XYZ = graphv8.l1_loss(XY-XYGT)/opt.batchSize
	loss_XYZ += graphv8.masked_l1_loss(depth-depthGT,maskLogit>0)/opt.batchSize
	loss_mask = graphv8.cross_entropy_loss(maskLogit,maskGT)/opt.batchSize
